...
Heatmap.Layer = OpenLayers.Class(OpenLayers.Layer, {
  ...
  initialize: function(name, options) {
    ...
    this.setGradientStops(this.cores);
  ...
  moveTo: function(bounds, zoomChanged, dragging) {
    ...
    for (var i in this.points) {
      ...
      var inteiro = src.intensity || this.defaultIntensity;
      var pos = this.map.getLayerPxFromLonLat(src.lonlat);
      var x = pos.x - rad + offsetX;
      var y = pos.y - rad + offsetY;

      if (!this.cache[inteiro]) {
        this.cache[inteiro] = {};
      }

      if (!this.cache[inteiro][rad]) {
        var grd = ctx.createRadialGradient(rad, rad, 0, rad, rad, rad);
        grd.addColorStop(0.0, 'rgba(0, 0, 0, ' + inteiro + ')');
        grd.addColorStop(1.0, 'transparent');
        this.cache[inteiro][rad] = grd;
      }

      ctx.fillStyle = this.cache[inteiro][rad];
      ctx.translate(x, y);
